...
func _ready():
	...
	for campaign_index in range(n_campaings):
		...
		$margin_container/vbox_container/campaigns_container.move_child(campaign_option, $margin_container/vbox_container/campaigns_container.get_child_count() - 2)
...
func _select_campaign(campaign_index):
	...
	campaign_manager.load_campaign_info(campaign_index)
